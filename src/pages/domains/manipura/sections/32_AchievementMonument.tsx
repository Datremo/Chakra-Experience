...
export const AchievementMonumentSection: React.FC = () => {
  ...
  return <WorldFrame eyebrow="17 — IDENTITY" title="What happens when achievement becomes identity?" copy="Build a monument from outcomes. Then remove the labels and see what remains." scene="human">
    <div className="grid lg:grid-cols-[.9fr_1.1fr] gap-5 items-stretch">
      <div className="rounded-[2rem] border border-amber-100/10 bg-amber-900/10 backdrop-blur-md border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.05)] p-6">
      ...
      <div className="relative rounded-[2rem] border border-amber-100/10 bg-amber-900/10 backdrop-blur-md border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.05)] min-h-[440px] overflow-hidden flex items-end justify-center p-10">
        ...
        <motion.div layout className="relative w-44 rounded-t-[3rem] bg-gradient-to-t from-amber-950/80 to-amber-300/20 border border-amber-200/10 flex flex-col-reverse items-center justify-end gap-1 p-3" style={{height: `${150 + added.length*34}px`, filter: stripped ? 'saturate(.45)' : 'none'}}>
          ...
          {added.map((x,i)=><motion.div key={x} layout className="w-full rounded-lg border border-amber-100/10 bg-amber-900/10 backdrop-blur-md border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.05)] py-2 text-center text-[9px] text-amber-50/55">{x}</motion.div>)}
        </motion.div>
