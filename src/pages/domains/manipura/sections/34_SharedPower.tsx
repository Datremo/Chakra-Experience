...
export const SharedPowerSection: React.FC = () => {
  ...
  return <WorldFrame eyebrow="19 — CAPACITY" title="Shared power is still power." copy="You do not have to carry every task to prove that you are capable." scene="human">
    <div className="grid md:grid-cols-5 gap-3 max-w-6xl mx-auto">{tasks.map((task,i)=><motion.button key={task} onClick={()=>assign(task)} whileTap={{scale:.98}} className={`relative min-h-[190px] rounded-[1.6rem] border p-4 text-left overflow-hidden ${assigned[task]?'border-emerald-200/25 bg-emerald-200/5':'border-amber-100/10 bg-black/25'}`}>
