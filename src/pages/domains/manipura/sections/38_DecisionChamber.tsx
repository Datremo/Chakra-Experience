...
export const DecisionChamberSection: React.FC = () => {
  ...
  return <WorldFrame eyebrow="23 — CHOICE" title="Between stimulus and response lies space." copy="A real-life choice: someone takes credit for your work. Notice the impulse, then choose the response you can own." scene="human">
    <div className="max-w-5xl mx-auto">
      <div className="rounded-[2rem] border border-amber-100/10 bg-amber-900/10 backdrop-blur-md border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.05)] p-8 md:p-10">
        ...
        <div className="grid md:grid-cols-4 gap-3 mt-8">{options.map(([key,label])=><button key={key} onClick={()=>choose(key)} className={`rounded-2xl border p-4 text-left ${picked===key?'border-amber-300/40 bg-amber-200/8':'border-amber-100/10 bg-amber-900/10 backdrop-blur-md border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.05)]'}`}><div className="text-sm text-amber-50/80">{label}</div><div className="mt-2 text-[10px] tracking-[.16em] uppercase text-amber-100/35">choose</div></button>)}</div>
        {picked && <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} className="mt-7 flex items-center gap-4 rounded-2xl border border-amber-100/10 bg-amber-900/10 backdrop-blur-md border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.05)] p-5"><div className="w-3 h-3 rounded-full bg-amber-300 shadow-[0_0_14px_rgba(255,190,75,.65)]"/><p className="text-sm text-amber-100/55">The point is not the “right” answer. The point is that the impulse did not get to become the only answer.</p></motion.div>}
      </div>
    </div>
  </WorldFrame>;
};
