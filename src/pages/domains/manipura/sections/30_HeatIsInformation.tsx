  ...
  return <WorldFrame eyebrow="15 — HEAT" title="Anger is heat, not a verdict." copy="A Maṇipūra experience can begin with heat. The work is learning what the heat is telling you before you decide what it means." scene="forge">
    <div className="max-w-5xl mx-auto">
      <div className="grid md:grid-cols-3 gap-3">
        {(['signal','story','choice'] as const).map(key => <button key={key} onClick={() => setMode(key)} className={`relative rounded-3xl border p-6 text-left transition-all ${mode === key ? 'border-amber-300/45 bg-amber-200/8 shadow-[0_0_50px_rgba(255,160,40,.08)]' : 'border-amber-100/10 bg-amber-900/10 backdrop-blur-md border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.05)]'}`}>
      ...
      <motion.div key={mode} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mt-5 rounded-[2rem] border border-amber-100/10 bg-amber-900/10 backdrop-blur-md border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.05)] p-7 text-center">
